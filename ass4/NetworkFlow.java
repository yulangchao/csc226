...
public class NetworkFlow{
	...
	*/
	static int[][] MaxFlow(int[][] G){
		...
		int rg[][]=new int[numVerts][numVerts];
		for(int i=0;i<numVerts;i++){
			for(int j=0;j<numVerts;j++){
				rg[i][j]=G[i][j];
			}
		}
		int rg1[][]=new int[numVerts][numVerts];
		int maxFlow=0;
		int path[]=new int[numVerts];
		while(hasP(rg,0,1,path,numVerts)){
			int minflow=Integer.MAX_VALUE;
			
			for(int k=1;k!=0;k=path[k]){
				int n=path[k];
				minflow=Math.min(minflow,rg[n][k]);
	...
	public static boolean hasP(int rg[][],int s,int t,int path[],int numVerts){
		boolean visit[]=new boolean[numVerts];
