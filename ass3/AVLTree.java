...
public class AVLTree{
	...
	*/
	...
	public int Balance(TreeNode a){
		int left,right;
		if(a.leftChild!=null){
			left=height(a.leftChild);
		}else{
			left=0;
		}
		if(a.rightChild!=null){
			right=height(a.rightChild);
		}else{
			right=0;
		}
		return right-left;
	...
	public void insert(TreeNode a, TreeNode rt){
		
		if(rt ==null){
			rt=a;
		}
		if(a.nodeValue.compareTo(rt.nodeValue)<=0){
			  ...
			  if(Balance(a)==-2){
                if(a.nodeValue.compareTo(rt.leftChild.nodeValue)<0)
                      root=LL(rt);
                else
                      root=LR(rt);				  
		     	}
		    	}else{
				insert(a,rt.leftChild);
		    	}
		}else{
			  ...
			  if(Balance(a)==2){
                if(a.nodeValue.compareTo(rt.rightChild.nodeValue)>0)
                      root=RR(rt);
                else
                      root=RL(rt);				  
		    	}	
		    	}else{
                insert(a,rt.rightChild);
		    	}			
		}	
		rt.height=Math.max(height(rt.leftChild),height(rt.rightChild))+1;
	...
	*/
	...
	private TreeNode LL(TreeNode r){
	TreeNode a=r.leftChild;
	r.leftChild=a.rightChild;
	a.rightChild=r;
	r.height=Math.max(height(r.leftChild), height(r.rightChild)) + 1;
	a.height=Math.max(height(a.leftChild), r.height) + 1;
	return a;
	}
	
	private TreeNode RR(TreeNode r){
	TreeNode a=r.rightChild;
	r.rightChild=a.leftChild;
	a.leftChild=r;
	r.height=Math.max(height(r.leftChild), height(r.rightChild)) + 1;
	a.height=Math.max(height(a.leftChild), r.height) + 1;
	return a;
	...
	private TreeNode LR(TreeNode a){
	a.leftChild = RR(a.leftChild);
	return LL(a);
	}
	
	private TreeNode RL(TreeNode a){
	a.rightChild = LL(a.rightChild);
	return RR(a);
	}
	...
	public int height(TreeNode a){
		...
		if(a.leftChild==null && a.rightChild==null) {
			...
			return 1+height(a.rightChild);
		} else if(a.rightChild==null) {
			return 1+height(a.leftChild);
		} else {
			h=Math.max(height(a.leftChild),height(a.rightChild))+1;
			return h;
		}
	}
